...
// 샘플 식당 데이터 추가
resList.push(new Restaurant("소갈비맛집", "수원시 권선구", "소갈비, 갈비"));
resList.push(new Restaurant("오리맛집", "전남 영광군", "오리 주물럭, 오리백숙"));
...
addBtn.addEventListener("click", () => {
    ...
    if(resName.value.trim().length == 0){
        alert("식당명을 입력해 주세요 제발 제발 제발 제발 제발 제발 제발");
        resName.focus();
        return; // 함수를 종료 시켜 아래 구문 수행 X
    }

    if(resAddress.value.trim().length == 0){
        alert("주소를 입력해 주세요 제발 제발 제발 제발 제발 제발 제발");
        resAddress.focus();
        return; // 함수를 종료 시켜 아래 구문 수행 X
    }

    if(resMenu.value.trim().length == 0){
        alert("메뉴를 입력해 주세요 제발 제발 제발 제발 제발 제발 제발");
        resMenu.focus();
        return; // 함수를 종료 시켜 아래 구문 수행 X
    }
...
/* 조회 버튼 클릭 시 #printList의 이전 내용을 모두 삭제하고
  resList의 요소를 이용해 li 요소를 만들어 추가 */
