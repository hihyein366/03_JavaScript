...
btn1.addEventListener("click", function(){
    ...
    if(value.trim().length == 0){  // trim() : 공백제거
        console.log("숫자 입력해줘잉");
...
document.querySelector("#btn2a").addEventListener("click", function(){
    const arr1 = [10, 20, 40];
    const arr2 = ['push', 'pull', 'baka'];

    const temp = 100;

    /* 참조란? 
       - 객체 지향 언어에서 배열, 객체 등 object는 값을 여러개를 가지고 있는
        자료형인데 변수는 값을 1개만 저장할수 있기 때문에 object를 메모리 다른
        곳에 저장하고 저장된 곳의 주소를 변수에 저장해서 필요할 때 마다 해당
        주소를 찾아가 원하는 겂을 얻어오거나 수정할수 있게 하는것
    ...
    // -> arr2에 저장된 주소만 보내서 함수를 수정했기 때문에

    // arr2가 참고하고 있던 배열의 2번 인덱스 내용이 변하게된다.
    //  -> 함수 수행 후 돌아와서도 참조하고 있던 배열의 2번 값 변화되어 있음.
...
/* 매개 변수로 요소 전달 */
function btn2bFn(el){
    // 매개 변수 el (element) : 이벤트가 발생한 요소(HTML 에서 this)
...
// 전달 받은 수(num)를 x제곱 하여 반환
function pow(num, x){
    let result = 1; // 곱했을 때 결과에 영향 주지 않기 위해 1로 초기화.
...
(() => {
    ...
    console.log("함수 모양 어렵네요 하하");
    console.log("따로 실행하지 않아도 콘솔에 바로 뜬답니다");
})();